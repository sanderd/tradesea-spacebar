...
// @match        https://app.tradesea.ai/account-center*
// @grant        none
...
(function () {
  ...
  function getNicknameMap() {
    return (userConfig && userConfig.accountNicknames) ? userConfig.accountNicknames : {};
  }
  ...
  function setNickname(brokerName, accountName, nickname) {
    ...
    if (!userConfig.accountNicknames) userConfig.accountNicknames = {};
    const map = userConfig.accountNicknames;
    const key = brokerName + ':' + accountName;
    if (!nickname) {
      delete map[key];
    } else {
  ...
  function openSettings() {
    ...
    settingsOverlay.querySelector('#ts-sb-save').addEventListener('click', () => {
      ...
      newCfg.accountNicknames = userConfig?.accountNicknames || {};
      saveConfig(newCfg);
      userConfig = newCfg;
      closeSettings();
    });
    ...
    const nickCountEl = settingsOverlay.querySelector('#ts-sb-nick-count');
    if (nickCountEl) nickCountEl.textContent = nickCount > 0 ? `${nickCount} nickname${nickCount !== 1 ? 's' : ''} configured` : 'No nicknames configured';
    settingsOverlay.querySelector('#ts-sb-clear-nicknames')?.addEventListener('click', () => {
